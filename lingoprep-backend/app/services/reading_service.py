...
def _toefl_reading_raw_to_scaled(correct: int, total: int) -> float:
    """Convert TOEFL iBT Reading raw score to 0-30 scaled score."""
    if total <= 0:
        return 0.0
    ratio = correct / total
    # TOEFL iBT Reading: 0-30 scale, roughly linear with slight curve
    if ratio >= 1.0: return 30.0
    elif ratio >= 0.95: return 29.0
    elif ratio >= 0.90: return 28.0
    elif ratio >= 0.85: return 27.0
    elif ratio >= 0.80: return 25.0
    elif ratio >= 0.75: return 24.0
...
def score_full_test(submission: FullTestSubmission, user_id: str = None) -> FullTestResult:
    ...
    if exam_type == "toefl":
        band_score = _toefl_reading_raw_to_scaled(correct_count, total)
        max_score = 30.0
    else:
